  ...
  contactForm.addEventListener('submit', (e) => {
    ...
    if (name && phone && msg) {
      ...
      const sendWa = confirm(
        currentLang === 'tr'
          ? `Teşekkürler Sayın ${name}. Mesajınızı İlhan Bey'e doğrudan WhatsApp üzerinden de iletmek ister misiniz?`
...
const translations = {
  tr: {
    ...
    "hero-desc": "İlhan Kurt güvencesiyle; Etimesgut, Bağlıca, Eryaman ve tüm Ankara aksında satılık ve kiralık konut, ticari mülk ve yatırımlık arsalarda dürüst esnaflık ve profesyonel\u00A0danışmanlık.",
    ...
    "sec2-title": "Ticari Gayrimenkul & Dükkân",
    "sec2-desc": "Cadde üzeri dükkânlar, mağazalar, plazalar, yüksek kira getirili kurumsal kiracılı ticari mülk yatırımları.",
    ...
    "footer-text": "Etimesgut Emlak Ofisi, İlhan Kurt güvencesiyle Ankara Etimesgut genelinde güvenilir, şeffaf ve profesyonel gayrimenkul alım-satım ve kiralama danışmanlığı sunar."
  },
  en: {
    ...
    "hero-desc": "Under the assurance of İlhan Kurt; providing honest, reliable and professional real estate solutions for residential, commercial and land investments in Etimesgut, Baglica, and Eryaman.",
    ...
    "footer-text": "Etimesgut Real Estate Office provides dependable, transparent and expert real estate brokerage led by İlhan Kurt in Ankara."
  }
};
  ...
  if (stored) {
    try {
      ...
      if (Array.isArray(parsed) && parsed.length > 0) {
        window.blogPostsData = parsed;
        return;
      }
    } catch (e) {
      console.error('LocalStorage blog parse error:', e);
    }
  }
  // İlk çalıştırmada varsayılan 30 blog verisini localStorage'a kaydet
  localStorage.setItem(STORAGE_KEY, JSON.stringify(window.blogPostsData));
}

// Ana sitedeki blog kartlarını dinamik render etme
function renderBlogGrid(activeFilter = 'all') {
  ...
  window.blogPostsData.forEach(post => {
    const isVisible = (activeFilter === 'all' || post.cat_slug === activeFilter);
    ...
    card.innerHTML = `
      <div class="blog-card-img">
        <img src="${post.img || 'images/etimesgut-emlak-logo.webp'}" alt="${post.title}" loading="lazy" onerror="this.src='images/etimesgut-emlak-logo.webp'">
        <span class="blog-category-badge">${post.category}</span>
      </div>
      <div class="blog-card-body">
        <div class="blog-card-meta">
          <span class="blog-meta-item">📅 ${post.date}</span>
          <span class="blog-meta-item">⏱ ${post.read_time || '5 dk okuma'}</span>
        </div>
...
function handleAdminLogin() {
  ...
  if (user === 'admin' && pass === 'etimesgut2026') {
    ...
    if (alertBox) {
      alertBox.innerText = 'Kullanıcı adı veya şifre hatalı!';
...
function renderAdminBlogTable(filterText = '', categoryFilter = 'all') {
  ...
  const countBadge = document.getElementById('admin-post-count');
  if (!tbody) return;
  ...
  if (filtered.length === 0) {
    tbody.innerHTML = '<tr><td colspan="6" style="text-align: center; padding: 2.5rem; color: var(--text-muted);">Eşleşen blog yazısı bulunamadı.</td></tr>';
    return;
  }
...
function filterAdminBlogTable() {
  const text = document.getElementById('admin-search-input')?.value || '';
  const cat = document.getElementById('admin-cat-filter')?.value || 'all';
  renderAdminBlogTable(text, cat);
}

function openAdminCreateModal() {
  document.getElementById('admin-modal-heading').innerText = '➕ Yeni Blog Yazısı Ekle';
  document.getElementById('admin-form-post-id').value = '';
  document.getElementById('admin-form-title').value = '';
  document.getElementById('admin-form-cat').value = 'rehber';
  
  const now = new Date();
  const trMonths = ['Ocak', 'Şubat', 'Mart', 'Nisan', 'Mayıs', 'Haziran', 'Temmuz', 'Ağustos', 'Eylül', 'Ekim', 'Kasım', 'Aralık'];
  const formattedDate = `${now.getDate()} ${trMonths[now.getMonth()]} ${now.getFullYear()}`;
  
  document.getElementById('admin-form-date').value = formattedDate;
  document.getElementById('admin-form-readtime').value = '5 dk okuma';
  document.getElementById('admin-form-img').value = 'images/etimesgut-emlak-logo.webp';
...
function openAdminEditModal(postId) {
  const post = window.blogPostsData.find(p => p.id === postId);
  if (!post) return;
  
  document.getElementById('admin-modal-heading').innerText = '✏️ Blog Yazısını Düzenle';
  document.getElementById('admin-form-post-id').value = post.id;
  document.getElementById('admin-form-title').value = post.title || '';
  document.getElementById('admin-form-cat').value = post.cat_slug || 'rehber';
  document.getElementById('admin-form-date').value = post.date || '';
  document.getElementById('admin-form-readtime').value = post.read_time || '5 dk okuma';
  document.getElementById('admin-form-img').value = post.img || '';
...
function deleteAdminBlogPost(postId) {
  ...
  const ok = confirm(`"${post.title}" başlıklı blog yazısını silmek istediğinize emin misiniz?`);
  if (!ok) return;
...
function resetAdminBlogData() {
  const ok = confirm('DİKKAT: Eklenen/düzenlenen yazılar sıfırlanıp varsayılan 30 orijinal blog yazısına geri dönülecektir. Onaylıyor musunuz?');
  if (!ok) return;
